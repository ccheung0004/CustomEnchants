    ...
    public class CustomEnchantmentListener implements Listener {
        ...
        private static final UUID DISABLED_WORLD_UUID = UUID.fromString("a0ece142-b6cb-44be-86f9-1b5e4f8acc4d");
        ...
        private void applyEnchantmentToItem(Player player, ItemStack currentItem, ItemStack cursorItem, CustomEnchantment enchantment) {
            ...
            // Sort the lore based on enchantment color codes
            lore.sort((a, b) -> {
                String colorA = a.substring(0, 2); // Get color code (e.g., "§b")
                String colorB = b.substring(0, 2);

                // Get the order index for each color
                int indexA = ENCHANTMENT_ORDER.indexOf(colorA);
                int indexB = ENCHANTMENT_ORDER.indexOf(colorB);

                // Sort by index, and if equal, sort alphabetically
                if (indexA != indexB) {
        ...
        private static final List<String> ENCHANTMENT_ORDER = Arrays.asList("§b", "§6", "§e", "§9", "§a");
        ...
        public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
            if (event.getEntity() instanceof Player && event.getDamager() instanceof Player) {
                ...
                if (damagedPlayer.getWorld().getUID().equals(DISABLED_WORLD_UUID) || damagerPlayer.getWorld().getUID().equals(DISABLED_WORLD_UUID)) {
                    return; // Do not apply custom enchantments in this world
                }
                ...
                // Handle bow damage reduction for HeavyEnchantment
                if (event.getDamager() instanceof Projectile) {
                    ...
                    if (chestplate != null && chestplate.hasItemMeta()) {
                        CustomEnchantmentManager.getAllEnchantments().forEach(enchantment -> {
                            if (enchantment instanceof HeavyEnchantment && enchantment.isAppliedTo(chestplate)) {
                                ((HeavyEnchantment) enchantment).handleBowDamageReduction(damagedPlayer, event);
                            }
                        });
                    }
                }

                // Handle fall damage for JellyLegsEnchantment
                if (event.getCause() == EntityDamageEvent.DamageCause.FALL) {
                    for (ItemStack armorPiece : damagedPlayer.getInventory().getArmorContents()) {
                        if (armorPiece != null && armorPiece.hasItemMeta()) {
                            CustomEnchantment enchantment = getEnchantmentFromItem(armorPiece);
                            if (enchantment instanceof JellyLegsEnchantment) {
                                ((JellyLegsEnchantment) enchantment).handleFallDamage(damagedPlayer);
                                break;
                            }
                        }
                    }
        ...
        public void onPlayerInteract(PlayerInteractEvent event) {
            ...
            if (player.getWorld().getUID().equals(DISABLED_WORLD_UUID)) {
                return; // Do not apply custom enchantments in this world
            }
